...
hfs_getattrlist(struct vop_getattrlist_args *ap)
{
	...
	attrbufptr = (void *)malloc(attrblocksize, M_TEMP, M_WAITOK);
	attrptr = attrbufptr;
...
hfs_readdirattr(struct vop_readdirattr_args *ap)
{
	...
	*(ap->a_eofflag) = 0;
	...
	attrbufptr = (void *)malloc(maxattrblocksize, M_TEMP, M_WAITOK);
	attrptr = attrbufptr;
	varptr = (char *)attrbufptr + fixedblocksize; /* Point to variable-length storage */

	/* Initialize a catalog entry list. */
	ce_list = (struct cat_entrylist *)malloc(sizeof(*ce_list), M_TEMP, M_WAITOK);
	...
	while (!depleted) {
		...
		if (index < dir_entries && !(*(ap->a_eofflag)) && lastdescp != NULL) {
			if (prevnamebuf == NULL)
				prevnamebuf = (char *)malloc(kHFSPlusMaxFileNameBytes + 1, M_TEMP, M_WAITOK);
