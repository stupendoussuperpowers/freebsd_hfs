...
__private_extern__ int static hfs_inactive(struct vop_inactive_args* ap)
{
  ...
  if ((cp->c_flag & C_DELETED) && (forkcount <= 1)) {
    ...
    if (error == 0) {
      ...
      (void)cat_update(
        hfsmp, &hfsmp->hfs_privdir_desc, &hfsmp->hfs_privdir_attr, NULL, NULL);
    }
...
hfs_getcnode(struct hfsmount* hfsmp,
             cnid_t cnid,
             ...
             struct vnode** vpp)
{
  ...
  if (descp != NULL) {
    ...
    if (retval == 0 && new_vp && (VTOC(new_vp)->c_flag & C_HARDLINK) &&
        cndesc.cd_nameptr && cndesc.cd_namelen > 0) {
...
#define FIFO 0
  ...
  if (cp == NULL) {
    cp2 = (struct cnode*)MALLOC_ZONE(sizeof(struct cnode), M_HFSNODE, M_WAITOK);
  ...
  if (IFTOVT(cp->c_mode) != VDIR) {
     ...
     */
    fp = (struct filefork*)MALLOC_ZONE(
      sizeof(struct filefork), M_HFSFORK, M_WAITOK);
