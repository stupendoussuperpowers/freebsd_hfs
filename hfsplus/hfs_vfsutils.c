...
char hfs_privdirname[] = "\xE2\x90\x80\xE2\x90\x80\xE2\x90\x80\xE2\x90\x80HFS+ Private Data";

OSErr
hfs_MountHFSVolume(struct hfsmount *hfsmp, HFSMasterDirectoryBlock *mdb, proc_t *p)
{
	...
	if (SWAP_BE32(mdb->drAlBlkSiz) == 0 || (SWAP_BE32(mdb->drAlBlkSiz) & 0x01FF) != 0)
		return (EINVAL);
	...
	error = hfs_getnewvnode(hfsmp, NULL, &cndesc, 0, &cnattr, &fork, &vcb->extentsRefNum);
	if (error)
		goto MtVolErr;

	error = MacToVFSError(
	    BTOpenPath(VTOF(vcb->extentsRefNum), (KeyCompareProcPtr)CompareExtentKeys, GetBTreeBlock, ReleaseBTreeBlock, ExtendBTreeFile, SetBTreeBlockSize));
	if (error) {
	...
	error = hfs_getnewvnode(hfsmp, NULL, &cndesc, 0, &cnattr, &fork, &vcb->catalogRefNum);
	if (error) {
		VOP_UNLOCK(vcb->extentsRefNum);
		goto MtVolErr;
	}
	...
	error = MacToVFSError(
	    BTOpenPath(VTOF(vcb->catalogRefNum), (KeyCompareProcPtr)CompareCatalogKeys, GetBTreeBlock, ReleaseBTreeBlock, ExtendBTreeFile, SetBTreeBlockSize));
	if (error) {
	...
	if (error == noErr) {
		if (!(vcb->vcbAtrb & kHFSVolumeHardwareLockMask)) //	if the disk is
								  // not write protected
		{
...
OSErr
hfs_MountHFSPlusVolume(struct hfsmount *hfsmp, HFSPlusVolumeHeader *vhp, off_t embeddedOffset, u_int64_t disksize, proc_t *p, void *args)
{
	ExtendedVCB *vcb;
	struct cat_desc cndesc;
	struct cat_attr cnattr;
	...
	if ((SWAP_BE16(vhp->signature) != kHFSPlusSigWord && SWAP_BE16(vhp->signature) != kHFSJSigWord) || SWAP_BE16(vhp->version) != kHFSPlusVersion) {
		// XXXdbg
	...
	if ((disksize & (hfsmp->hfs_phys_block_size - 1)) || (embeddedOffset & (hfsmp->hfs_phys_block_size - 1)) ||
	...
	retval = hfs_getnewvnode(hfsmp, NULL, &cndesc, 0, &cnattr, (struct cat_fork *)&vhp->extentsFile, &vcb->extentsRefNum);
	...
	retval = MacToVFSError(BTOpenPath(VTOF(vcb->extentsRefNum), (KeyCompareProcPtr)CompareExtentKeysPlus, GetBTreeBlock, ReleaseBTreeBlock, ExtendBTreeFile,
	    SetBTreeBlockSize));
	...
	retval = hfs_getnewvnode(hfsmp, NULL, &cndesc, 0, &cnattr, (struct cat_fork *)&vhp->catalogFile, &vcb->catalogRefNum);
	...
	retval = MacToVFSError(BTOpenPath(VTOF(vcb->catalogRefNum), (KeyCompareProcPtr)CompareExtendedCatalogKeys, GetBTreeBlock, ReleaseBTreeBlock,
	...
	retval = hfs_getnewvnode(hfsmp, NULL, &cndesc, 0, &cnattr, (struct cat_fork *)&vhp->allocationFile, &vcb->allocationsRefNum);
	SWAP_HFS_PLUS_FORK_DATA(&vhp->allocationFile);
	if (retval) {
		VOP_UNLOCK(vcb->catalogRefNum);
		VOP_UNLOCK(vcb->extentsRefNum);
 ...
 */
static void
ReleaseMetaFileVNode(struct vnode *vp)
{
	struct filefork *fp;
...
short
hfsUnmount(register struct hfsmount *hfsmp, proc_t *p)
{
	ExtendedVCB *vcb = HFSTOVCB(hfsmp);
	int retval = E_NONE;
...
overflow_extents(struct filefork *fp)
{
	...
	if (VTOVCB(FTOV(fp))->vcbSigWord == kHFSPlusSigWord) {
		...
		blocks = fp->ff_extents[0].blockCount + fp->ff_extents[1].blockCount + fp->ff_extents[2].blockCount + fp->ff_extents[3].blockCount +
		    fp->ff_extents[4].blockCount + fp->ff_extents[5].blockCount + fp->ff_extents[6].blockCount + fp->ff_extents[7].blockCount;
	} else {
		if (fp->ff_extents[2].blockCount == 0)
			return false;

		blocks = fp->ff_extents[0].blockCount + fp->ff_extents[1].blockCount + fp->ff_extents[2].blockCount;
	}
...
int
hfs_metafilelocking(struct hfsmount *hfsmp, u_long fileID, u_int flags, proc_t *p)
{
	ExtendedVCB *vcb;
	...
	if ((flags & LK_TYPE_MASK) == LK_RELEASE) {
		...
		if (buf_dirty_count_severe() || ((tv.tv_sec - lastfsync) > kMaxSecsForFsync)) {
			hfs_btsync(vp, HFS_SYNCTRANS);
		}

		flags &= ~LK_RELEASE;
		retval = VOP_UNLOCK(vp);
		if (vp2 && retval == 0) {
			retval = VOP_UNLOCK(vp2);
		}
	} else {
...
#if HFS_DIAGNOSTIC
void
RequireFileLock(FileReference vp, int shareable)
{
	int locked = false;

	switch (lockstatus(vp->v_vnlock)) {
	case 0:
	case LK_EXCLOTHER:
		break;
	case LK_SHARED:
	...
	case LK_EXCLUSIVE:
		...
		break;
	default:
		panic("RequireFileLock: unexpected result from lockstatus()");
	}
	...
	if (!locked) {
		...
		case 3:
			// DEBUG_BREAK_MSG(
			//     (" #\n # RequireFileLock: extent btree vnode not locked! v: "
			//      "0x%08X\n #\n",
			//      (u_int)vp));
			break;
		...
		case 4:
			// DEBUG_BREAK_MSG(
			//     (" #\n # RequireFileLock: catalog btree vnode not locked! v: "
			//      "0x%08X\n #\n",
			//      (u_int)vp));
			break;
 ...
 */
int
hfs_owner_rights(struct hfsmount *hfsmp, uid_t cnode_uid, struct ucred *cred, int invokesuperuserstatus)
{
...
unsigned long
BestBlockSizeFit(unsigned long allocationBlockSize, unsigned long blockSizeLimit, unsigned long baseMultiple)
{
	/*
	   Compute the optimal (largest) block size (no larger than
	   allocationBlockSize) that is less than the specified limit but still an
	   even multiple of the baseMultiple.
	 */
	...
	if (allocationBlockSize % baseMultiple != 0) {
		/*
		   Whoops: the allocation blocks aren't even multiples of the specified
		   base: no amount of dividing them into even parts will be a multiple,
		   either then!
		*/
		return 512; /* Hope for the best */
	};

	/* Try the obvious winner first, to prevent 12K allocation blocks, for
	   instance, from being handled as two 6K logical blocks instead of 3 4K
	   logical blocks. Even though the former (the result of the loop below) is
	   the larger allocation block size, the latter is more efficient: */
	if (allocationBlockSize % PAGE_SIZE == 0)
		return PAGE_SIZE;
	...
	for (blockCount = baseBlockCount; blockCount > 0; --blockCount) {
		...
		if (allocationBlockSize % trialBlockSize == 0) { /* An even multiple? */
			if ((trialBlockSize <= blockSizeLimit) && (trialBlockSize % baseMultiple == 0)) {
...
FindMetaDataDirectory(ExtendedVCB *vcb)
{
	...
	/* Lock catalog b-tree */
	error = hfs_metafilelocking(hfsmp, kHFSCatalogFileID, LK_EXCLUSIVE, curthread);
	if (error)
		return (0);

	error = cat_lookup(hfsmp, &hfsmp->hfs_privdir_desc, 0, NULL, &hfsmp->hfs_privdir_attr, NULL);

	if (error == 0) {
		/* Unlock catalog b-tree */
		(void)hfs_metafilelocking(hfsmp, kHFSCatalogFileID, LK_RELEASE, curthread);
	...
	hfsmp->hfs_privdir_attr.ca_mtime = gettime();
	...
	error = cat_create(hfsmp, &hfsmp->hfs_privdir_desc, &hfsmp->hfs_privdir_attr, &out_desc);
...
u_long
GetFileInfo(ExtendedVCB *vcb, u_int32_t dirid, char *name, struct cat_attr *fattr, struct cat_fork *forkinfo)
{
	// Callers expect 0 on error.
...
hfs_remove_orphans(struct hfsmount *hfsmp)
{
	...
	iterator = (struct BTreeIterator *)malloc(sizeof(*iterator), M_TEMP, M_WAITOK);
	...
	/* Lock catalog b-tree */
	result = hfs_metafilelocking(hfsmp, kHFSCatalogFileID, LK_EXCLUSIVE, curthread);
	if (result)
		goto exit;
	catlock = 1;
	...
	for (;;) {
		...
		(void)utf8_encodestr(keyp->nodeName.unicode, keyp->nodeName.length * 2, filename, &namelen, sizeof(filename), 0, 0);
		...
		if (bcmp(tempname, filename, namelen) == 0) {
			...
			cnode.c_blocks = filerec.dataFork.totalBlocks + filerec.resourceFork.totalBlocks;

			/* Position iterator at previous entry */
			if (BTIterateRecord(fcb, kBTreePrevRecord, iterator, NULL, NULL) != 0)
				break;
...
GetLogicalBlockSize(struct vnode *vp)
{
	...
	if (vp->v_vflag & VV_SYSTEM) {
		if (VTOF(vp)->fcbBTCBPtr != NULL) {
			...
			/*
			 * We do not lock the BTrees, because if we are getting block..then the
			 * tree should be locked in the first place. We just want the nodeSize
			 * wich will NEVER change..so even if the world is changing..the nodeSize
			 * should remain the same. Which argues why lock it in the first place??
			 */
...
hfs_getnamehint(struct cnode *dcp, int index)
{
	...
	if (index > 0) {
		...
		SLIST_FOREACH(entry, &dcp->c_indexlist, hi_link) {
			if ((entry->hi_index == index) && (entry->hi_thread == self))
...
hfs_savenamehint(struct cnode *dcp, int index, const char *namehint)
{
	...
	if (index > 0) {
		len = strlen(namehint);
		entry = (struct hfs_index *)malloc(len + sizeof(struct hfs_index), M_TEMP, M_WAITOK);
		entry->hi_index = index;
		entry->hi_thread = curthread;
...
hfs_namecmp(const char *str1, size_t len1, const char *str2, size_t len2)
{
	...
	ustr1 = (u_int16_t *)malloc(maxbytes << 1, M_TEMP, M_WAITOK);
...
#ifdef DARWIN_JOURNAL
int
hfs_early_journal_init(struct hfsmount *hfsmp, HFSPlusVolumeHeader *vhp, void *_args, int embeddedOffset, int mdb_offset, HFSMasterDirectoryBlock *mdbp,
    struct ucred *cred)
{
	...
	retval = bread(devvp, embeddedOffset / blksize + (SWAP_BE32(vhp->journalInfoBlock) * sectors_per_fsblock), SWAP_BE32(vhp->blockSize), cred, &jinfo_bp);
	if (retval)
		return retval;
	...
	if (jibp->flags & kJIJournalNeedInitMask) {
		...
		hfsmp->jnl = journal_create(hfsmp->jvp, jibp->offset + (off_t)embeddedOffset, jibp->size, devvp, blksize, arg_flags, arg_tbufsz,
		...
		// printf("hfs: Opening the journal (joffset 0x%llx sz 0x%llx vhp_blksize
		// %d)...\n", 	   jibp->offset + (off_t)embeddedOffset,
		// jibp->size,
		// SWAP_BE32(vhp->blockSize));

		hfsmp->jnl = journal_open(hfsmp->jvp, jibp->offset + (off_t)embeddedOffset, jibp->size, devvp, blksize, arg_flags, arg_tbufsz,
		    hfs_sync_metadata, hfsmp->hfs_mp);
		...
		if (hfsmp->jnl && mdbp) {
			...
			if (retval) {
				brelse(bp);
				printf("hfs: failed to reload the mdb after opening the journal (retval "
				       "%d)!\n",
				    retval);
				return retval;
			}
...
hfs_late_journal_init(struct hfsmount *hfsmp, HFSPlusVolumeHeader *vhp, void *_args)
{
	...
	if (fid == 0 || jib_fork.cf_extents[0].startBlock == 0 || jib_fork.cf_size == 0) {
		printf("hfs: can't find the .journal_info_block! disabling journaling (start: "
		       "%d).\n",
	...
	retval = bread(devvp, vcb->hfsPlusIOPosOffset / hfsmp->hfs_phys_block_size + (SWAP_BE32(vhp->journalInfoBlock) * sectors_per_fsblock),
	...
	if (jibp->size != (u_int64_t)jfork.cf_extents[0].blockCount * vcb->blockSize) {
		printf("hfs: The journal file changed size! (was %lld; is %lld).  Fixing "
		       "up.\n",
	...
	if (jibp->flags & kJIJournalNeedInitMask) {
		...
		hfsmp->jnl = journal_create(hfsmp->jvp, jibp->offset + (off_t)vcb->hfsPlusIOPosOffset, jibp->size, devvp, hfsmp->hfs_phys_block_size, arg_flags,
		    arg_tbufsz, hfs_sync_metadata, hfsmp->hfs_mp);
		...
		hfsmp->jnl = journal_open(hfsmp->jvp, jibp->offset + (off_t)vcb->hfsPlusIOPosOffset, jibp->size, devvp, hfsmp->hfs_phys_block_size, arg_flags,
		    arg_tbufsz, hfs_sync_metadata, hfsmp->hfs_mp);
	}
