...
static int hfs_late_journal_init(struct hfsmount* hfsmp,
...
extern OSStatus GetBTreeBlock(FileReference vp,
                              UInt32 blockNum,
                              GetBlockOptions options,
                              BlockDescriptor* block);
extern OSStatus SetBTreeBlockSize(FileReference vp,
                                  ByteCount blockSize,
                                  ItemCount minBlockCount);
extern OSStatus ExtendBTreeFile(FileReference vp, FSSize minEOF, FSSize maxEOF);
extern OSStatus ReleaseBTreeBlock(FileReference vp,
...
OSErr hfs_MountHFSVolume(struct hfsmount* hfsmp,
  ...
  hfsmp->hfs_logBlockSize =
      BestBlockSizeFit(vcb->blockSize, MAXBSIZE, hfsmp->hfs_phys_block_size);
...
OSErr hfs_MountHFSPlusVolume(struct hfsmount* hfsmp,
  ...
  if (!(vcb->vcbAtrb &
        kHFSVolumeHardwareLockMask))  // if the disk is not write protected
  {
    MarkVCBDirty(vcb);  // mark VCB dirty so it will be written
  }
...
short hfsUnmount(register struct hfsmount* hfsmp, proc_t* p) {
  ExtendedVCB* vcb = HFSTOVCB(hfsmp);
  int retval = E_NONE;
 ...
 */
static short hfs_getcatalog(void* p1, u_long p2, void* p3, short p4, void* p5) {
  return ENOENT;
}
...
static int overflow_extents(struct filefork* fp) {
  ...
  if (VTOVCB(FTOV(fp))->vcbSigWord == kHFSPlusSigWord) {
    ...
    blocks = fp->ff_extents[0].blockCount + fp->ff_extents[1].blockCount +
             fp->ff_extents[2].blockCount + fp->ff_extents[3].blockCount +
             fp->ff_extents[4].blockCount + fp->ff_extents[5].blockCount +
             fp->ff_extents[6].blockCount + fp->ff_extents[7].blockCount;
  } else {
...
/* __private_extern__ */
int hfs_metafilelocking(struct hfsmount* hfsmp,
                        u_long fileID,
                        u_int flags,
                        proc_t* p) {
  ExtendedVCB* vcb;
  ...
  if ((flags & LK_TYPE_MASK) == LK_RELEASE) {
    ...
    if ((numOfLockedBuffs > kMaxLockedMetaBuffers) ||
        ((numOfLockedBuffs > 1) &&
         ((tv.tv_sec - lastfsync) > kMaxSecsForFsync))) {
#else
...
unsigned long BestBlockSizeFit(unsigned long allocationBlockSize,
  ...
  baseBlockCount = allocationBlockSize /
                   baseMultiple; /* Now guaranteed to be an even multiple */
...
u_long FindMetaDataDirectory(ExtendedVCB* vcb) {
  ...
  error = cat_create(hfsmp, &hfsmp->hfs_privdir_desc, &hfsmp->hfs_privdir_attr,
                     &out_desc);
...
__private_extern__ u_long GetFileInfo(ExtendedVCB* vcb,
                                      u_int32_t dirid,
                                      char* name,
                                      struct cat_attr* fattr,
                                      struct cat_fork* forkinfo) {
 ...
 */
__private_extern__ void hfs_remove_orphans(struct hfsmount* hfsmp) {
  ...
  /* Lock catalog b-tree */
  result = hfs_metafilelocking(hfsmp, kHFSCatalogFileID, LK_EXCLUSIVE,
                               current_proc());
  if (result)
    goto exit;
  catlock = 1;
  ...
  for (;;) {
    ...
    if (bcmp(tempname, filename, namelen) == 0) {
      ...
      cnode.c_blocks =
          filerec.dataFork.totalBlocks + filerec.resourceFork.totalBlocks;
 ...
 */
__private_extern__ static char* hfs_getnamehint(struct cnode* dcp, int index) {
 ...
 * The directory cnode (dcp) must be locked.
 */
__private_extern__ static void hfs_savenamehint(struct cnode* dcp,
                                         int index,
                                         const char* namehint) {
  struct hfs_index* entry;
  int len;
  ...
  if (index > 0) {
    len = strlen(namehint);
    entry = (struct hfs_index*)MALLOC(len + sizeof(struct hfs_index), M_TEMP,
                                      M_WAITOK);
 ...
 */
__private_extern__ int hfs_namecmp(const char* str1,
                                   size_t len1,
                                   const char* str2,
                                   size_t len2) {
...
#ifdef DARWIN_JOURNAL
__private_extern__ int hfs_early_journal_init(struct hfsmount* hfsmp,
                                              HFSPlusVolumeHeader* vhp,
                                              void* _args,
                                              int embeddedOffset,
                                              int mdb_offset,
  ...
  if (jibp->flags & kJIJournalNeedInitMask) {
    ...
    hfsmp->jnl = journal_create(
        hfsmp->jvp, jibp->offset + (off_t)embeddedOffset, jibp->size, devvp,
        blksize, arg_flags, arg_tbufsz, hfs_sync_metadata, hfsmp->hfs_mp);
    ...
    hfsmp->jnl = journal_open(hfsmp->jvp, jibp->offset + (off_t)embeddedOffset,
                              jibp->size, devvp, blksize, arg_flags, arg_tbufsz,
    ...
    if (hfsmp->jnl && mdbp) {
      ...
      if (retval) {
        brelse(bp);
        printf(
            "hfs: failed to reload the mdb after opening the journal (retval "
            "%d)!\n",
            retval);
        return retval;
      }
...
static int hfs_late_journal_init(struct hfsmount* hfsmp,
  ...
  fid =
      GetFileInfo(vcb, kRootDirID, ".journal_info_block", &jib_attr, &jib_fork);
  if (fid == 0 || jib_fork.cf_extents[0].startBlock == 0 ||
      jib_fork.cf_size == 0) {
    printf(
        "hfs: can't find the .journal_info_block! disabling journaling (start: "
        "%d).\n",
  ...
  if ((jibp->offset / (u_int64_t)vcb->blockSize) !=
      jfork.cf_extents[0].startBlock) {
    ...
    jibp->offset =
        (u_int64_t)jfork.cf_extents[0].startBlock * (u_int64_t)vcb->blockSize;
    write_jibp = 1;
  }

  // check the size of the journal file.
  if (jibp->size !=
      (u_int64_t)jfork.cf_extents[0].blockCount * vcb->blockSize) {
    printf(
        "hfs: The journal file changed size! (was %lld; is %lld).  Fixing "
        "up.\n",
  ...
  if (jibp->flags & kJIJournalNeedInitMask) {
    ...
    hfsmp->jnl = journal_create(
        hfsmp->jvp, jibp->offset + (off_t)vcb->hfsPlusIOPosOffset, jibp->size,
        devvp, hfsmp->hfs_phys_block_size, arg_flags, arg_tbufsz,
        hfs_sync_metadata, hfsmp->hfs_mp);
    ...
    hfsmp->jnl =
        journal_open(hfsmp->jvp, jibp->offset + (off_t)vcb->hfsPlusIOPosOffset,
                     jibp->size, devvp, hfsmp->hfs_phys_block_size, arg_flags,
