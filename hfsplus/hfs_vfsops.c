...
static int
hfs_mountfs(struct vnode *devvp, struct mount *mp)
{
    proc_t *p = curthread;
    int retval = E_NONE;
    ...
    if (blksize > 512)
    {
        ...
        if (VOP_IOCTL(
                devvp, DKIOCSETBLOCKSIZE, (caddr_t)&size512, FWRITE, cred, p))
        {
            retval = ENXIO;
            goto error_exit;
        }
    ...
    {
        if (VOP_IOCTL(
                devvp, DKIOCSETBLOCKSIZE, (caddr_t)&blksize, FWRITE, cred, p))
        {
            retval = ENXIO;
            goto error_exit;
        }
        ...
        (mntwrapper || (SWAP_BE16(mdbp->drEmbedSigWord) != kHFSPlusSigWord)))
    {
        ...
        retval = hfs_getconverter(
            hfsmp->hfs_encoding, &hfsmp->hfs_get_unicode, &hfsmp->hfs_get_hfsname);
        if (retval)
            goto error_exit;
    ...
    else /* Mount an HFS Plus disk */
    {
        ...
        if (SWAP_BE16(mdbp->drEmbedSigWord) == kHFSPlusSigWord)
        {
            ...
            if ((embeddedOffset % blksize) != 0)
            {
                ...
                if (VOP_IOCTL(
                        devvp, DKIOCGETBLOCKCOUNT, (caddr_t)&blkcnt, 0, cred, p))
                {
                    retval = ENXIO;
                    goto error_exit;
                }
            ...
            !jnl_disable)
        {
            ...
            // point as journaled.
            //
            if (hfs_early_journal_init(
                    hfsmp, vhp, args, embeddedOffset, mdb_offset, mdbp, cred) == 0)
            {
    ...
    /*
        if (mp->mnt_flag & MNT_UPDATE)
        {
            ...
            if ((hfsmp->hfs_fs_ronly == 0) && (mp->mnt_flag & MNT_RDONLY))
            {
                ...
                retval = VFS_SYNC(mp, MNT_WAIT, p->td_proc->p_ucred, p);
                ...
                if (!retval)
                {
                    vn_lock(hfsmp->hfs_devvp,
                            LK_EXCLUSIVE | LK_RETRY); // VOP_FSYNC takes vp locked
            ...
            if (hfsmp->hfs_fs_ronly && (mp->mnt_kern_flag & MNTK_WANTRDWR))
            {
                ...
                if (p->td_proc->p_ucred->cr_uid != 0)
                {
                    ...
                    vn_lock(devvp, LK_EXCLUSIVE | LK_RETRY);
                    if ((retval =
                             VOP_ACCESS(devvp, VREAD | VWRITE, p->td_proc->p_ucred, p)))
                    {
                        VOP_UNLOCK(devvp);
                        goto error_exit;
                    }
        ...
        {
            DBG_ERR((
                "hfs_mount: CAN'T GET DEVICE: %s, %x\n", args.fspec, ndp->ni_vp->v_rdev));
            goto error_exit;
        }
...
static int
hfs_root(struct mount *mp, int flags, struct vnode **vpp)
{
    printf("---hfs_root---\n");
...
static int
hfs_statfs(struct mount *mp, struct statfs *sbp)
{
    printf("---hfs_statfs---\n");
    sbp->f_bsize = 4096;
    sbp->f_blocks = 1000;
...
static int
hfs_vget(struct mount *mp, ino_t ino, int flags, struct vnode **vpp)
{
    cnid_t cnid = ino;
...
hfs_flushMDB(struct hfsmount *hfsmp, int waitfor, int altflush)
{
    ...
    retval = bread(
        hfsmp->hfs_devvp, HFS_PRI_SECTOR(sectorsize), sectorsize, NOCRED, &bp);
    if (retval)
    {
        if (bp)
            brelse(bp);
    ...
    if (altflush)
    {
        ...
        if (meta_bread(
                hfsmp->hfs_devvp, altIDSector, sectorsize, NOCRED, &alt_bp) == 0)
        {
...
int hfs_flushvolumeheader(struct hfsmount *hfsmp, int waitfor, int altflush)
{
        ...
        (SWAP_BE32(volumeHeader->createDate) != vcb->localCreateDate))
    {
        ...
        retval = meta_bread(
            hfsmp->hfs_devvp, HFS_PRI_SECTOR(sectorsize), sectorsize, NOCRED, &bp2);
        if (retval)
        {
            if (bp2)
                brelse(bp2);
...
//
    ...
    volumeHeader->attributes = SWAP_BE32(
        (SWAP_BE32(volumeHeader->attributes) & 0xFFFF0000) + (UInt16)vcb->vcbAtrb);
    ...
    if (bcmp(vcb->vcbFndrInfo,
             volumeHeader->finderInfo,
             sizeof(volumeHeader->finderInfo)) != 0)
    {
        // critical = 1;
        bcopy(vcb->vcbFndrInfo,
              volumeHeader->finderInfo,
    ...
    if (altflush)
    {
                ...
                hfsmp->hfs_devvp, altIDSector, sectorsize, NOCRED, &alt_bp) == 0)
        {
            ...
            bcopy(
                volumeHeader, alt_bp->b_data + HFS_ALT_OFFSET(sectorsize), kMDBSize);
