...
OSErr
ExchangeFileIDs(ExtendedVCB *vcb, ConstUTF8Param srcName, ConstUTF8Param destName, HFSCatalogNodeID srcID, HFSCatalogNodeID destID, UInt32 srcHint,
    UInt32 destHint)
{
...
void
CopyCatalogNodeInfo(CatalogRecord *src, CatalogRecord *dest)
{
...
void
CopyBigCatalogNodeInfo(CatalogRecord *src, CatalogRecord *dest)
{
...
OSErr
MoveExtents(ExtendedVCB *vcb, UInt32 srcFileID, UInt32 destFileID, Boolean isHFSPlus)
{
	FCB *fcb;
...
void
CopyExtentInfo(ExtentKey *key, ExtentRecord *data, ExtentsRecBuffer *buffer, UInt16 bufferCount)
{
...
OSErr
DeleteExtents(ExtendedVCB *vcb, UInt32 fileID, Boolean isHFSPlus)
{
	FCB *fcb;
	ExtentKey *extentKeyPtr;
	ExtentRecord extentData;
...
UInt32
CheckExtents(void *extents, UInt32 totalBlocks, Boolean isHFSPlus)
{
	...
	if (isHFSPlus) {
		for (i = 0; i < kHFSPlusExtentDensity; i++) {
			...
			if (extentAllocationBlocks >=
			    totalBlocks) //	greater than or equal (extents can add past eof if 'Close" crashes w/o truncating new clump)
				return (0);
		}
	} else {
