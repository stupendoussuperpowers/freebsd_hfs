...
enum { CMMaxCName = kHFSMaxFileNameChars };
...
#define VCB_LOCK_INIT(vcb)    mtx_init(&vcb->vcbSimpleLock, "hfs vcb lock", NULL, MTX_DEF)
#define VCB_LOCK(vcb)	      mtx_lock(&vcb->vcbSimpleLock)
#define VCB_UNLOCK(vcb)	      mtx_unlock(&vcb->vcbSimpleLock)
#define VCB_LOCK_DESTROY(vcb) mtx_destroy(&vcb->vcbSimpleLock)

#define MarkVCBDirty(vcb)                    \
	{                                    \
		((vcb)->vcbFlags |= 0xFF00); \
	}
#define MarkVCBClean(vcb)                    \
	{                                    \
		((vcb)->vcbFlags &= 0x00FF); \
	}
...
EXTERN_API_C(void)
ReturnErrorIf(Boolean condition, OSErr result);

#define ReturnErrorIf(condition, error) \
	if ((condition))                \
		return ((error));
...
EXTERN_API_C(OSErr)
ExchangeFileIDs(ExtendedVCB *volume, ConstUTF8Param srcName, ConstUTF8Param destName, HFSCatalogNodeID srcID, HFSCatalogNodeID destID, UInt32 srcHint,
    UInt32 destHint);
...
EXTERN_API_C(OSErr)
SearchBTreeRecord(FileReference refNum, const void *key, UInt32 hint, void *foundKey, void *data, UInt16 *dataSize, UInt32 *newHint);

EXTERN_API_C(OSErr)
ReplaceBTreeRecord(FileReference refNum, const void *key, UInt32 hint, void *newData, UInt16 dataSize, UInt32 *newHint);

/*	Prototypes for exported routines in VolumeAllocation.c*/
EXTERN_API_C(OSErr)
BlockAllocate(ExtendedVCB *vcb, UInt32 startingBlock, SInt64 bytesRequested, SInt64 bytesMaximum, Boolean forceContiguous, UInt32 *startBlock,
    UInt32 *actualBlocks);

EXTERN_API_C(OSErr)
BlockDeallocate(ExtendedVCB *vcb, UInt32 firstBlock, UInt32 numBlocks);

EXTERN_API_C(UInt32)
FileBytesToBlocks(SInt64 numerator, UInt32 denominator);
...
EXTERN_API_C(SInt32)
CompareExtentKeys(const HFSExtentKey *searchKey, const HFSExtentKey *trialKey);

EXTERN_API_C(SInt32)
CompareExtentKeysPlus(const HFSPlusExtentKey *searchKey, const HFSPlusExtentKey *trialKey);

EXTERN_API_C(OSErr)
TruncateFileC(ExtendedVCB *vcb, FCB *fcb, SInt64 peof, Boolean truncateToExtent);

EXTERN_API_C(OSErr)
ExtendFileC(ExtendedVCB *vcb, FCB *fcb, SInt64 bytesToAdd, UInt32 blockHint, UInt32 flags, SInt64 *actualBytesAdded);

EXTERN_API_C(OSErr)
MapFileBlockC(ExtendedVCB *vcb, FCB *fcb, size_t numberOfBytes, off_t offset, daddr_t *startBlock, size_t *availableBytes);

#if TARGET_API_MACOS_X
EXTERN_API_C(Boolean)
NodesAreContiguous(ExtendedVCB *vcb, FCB *fcb, UInt32 nodeSize);
#endif
