    ...
    UInt32 *returnNodeNum, UInt16 *returnIndex, Boolean *foundRecord)
{
	...
	if (((NodeDescPtr)middle->buffer)->kind != kBTLeafNode || ((NodeDescPtr)middle->buffer)->numRecords <= 0) {
		goto SearchTheTree;
	}
	...
	if (index == 0) {
		...
		if (((NodeDescPtr)left->buffer)->kind != kBTLeafNode || ((NodeDescPtr)left->buffer)->numRecords <= 0) {
			goto SearchTheTree;
		}
		...
		if (((NodeDescPtr)right->buffer)->kind != kBTLeafNode || ((NodeDescPtr)right->buffer)->numRecords <= 0) {
			goto SearchTheTree;
		}
	...
	*recordInserted = false; // we'll assume this won't work...
	...
	if (spaceNeeded == oldSpace) {
		...
		*recordInserted = true;
		...
		didItFit = InsertKeyRecord(btreePtr, nodePtr, index, &iterator->key, KeyLength(btreePtr, &iterator->key), record->bufferAddress, recordLen);
