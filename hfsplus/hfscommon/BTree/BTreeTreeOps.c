...
static OSStatus AddNewRootNode(BTreeControlBlockPtr btreePtr, NodeDescPtr leftNode, NodeDescPtr rightNode);

static OSStatus CollapseTree(BTreeControlBlockPtr btreePtr, BlockDescriptor *blockPtr);

static OSStatus RotateLeft(BTreeControlBlockPtr btreePtr, NodeDescPtr leftNode, NodeDescPtr rightNode, UInt16 rightInsertIndex, KeyPtr keyPtr, UInt8 *recPtr,
    UInt16 recSize, UInt16 *insertIndex, UInt32 *insertNodeNum, Boolean *recordFit, UInt16 *recsRotated);

static Boolean RotateRecordLeft(BTreeControlBlockPtr btreePtr, NodeDescPtr leftNode, NodeDescPtr rightNode);

static OSStatus SplitLeft(BTreeControlBlockPtr btreePtr, BlockDescriptor *leftNode, BlockDescriptor *rightNode, UInt32 rightNodeNum, UInt16 index,
    KeyPtr keyPtr, UInt8 *recPtr, UInt16 recSize, UInt16 *insertIndex, UInt32 *insertNodeNum, UInt16 *recsRotated);

static OSStatus InsertLevel(BTreeControlBlockPtr btreePtr, TreePathTable treePathTable, InsertKey *primaryKey, InsertKey *secondaryKey,
    BlockDescriptor *targetNode, UInt16 index, UInt16 level, UInt32 *insertNode);

static OSErr InsertNode(BTreeControlBlockPtr btreePtr, InsertKey *key, BlockDescriptor *rightNode, UInt32 node, UInt16 index, UInt32 *newNode, UInt16 *newIndex,
    BlockDescriptor *leftNode, Boolean *updateParent, Boolean *insertParent, Boolean *rootSplit);
...
OSStatus
SearchTree(BTreeControlBlockPtr btreePtr, BTreeKeyPtr searchKey, TreePathTable treePathTable, UInt32 *nodeNum, BlockDescriptor *nodePtr, UInt16 *returnIndex)
{
	OSStatus err;
...
OSStatus
InsertTree(BTreeControlBlockPtr btreePtr, TreePathTable treePathTable, KeyPtr keyPtr, UInt8 *recPtr, UInt16 recSize, BlockDescriptor *targetNode, UInt16 index,
    UInt16 level, Boolean replacingKey, UInt32 *insertNode)
{
	...
	err = InsertLevel(btreePtr, treePathTable, &primaryKey, nil, targetNode, index, level, insertNode);
...
OSStatus
InsertLevel(BTreeControlBlockPtr btreePtr, TreePathTable treePathTable, InsertKey *primaryKey, InsertKey *secondaryKey, BlockDescriptor *targetNode,
    UInt16 index, UInt16 level, UInt32 *insertNode)
{
	OSStatus err;
	...
	err = InsertNode(btreePtr, primaryKey, targetNode, targetNodeNum, index, &newNodeNum, &newIndex, &leftNode, &updateParent, &insertParent, &newRoot);
	M_ExitOnError(err);
	...
	if (secondaryKey != nil) {
		...
		err = InsertNode(btreePtr, secondaryKey, targetNode, newNodeNum, newIndex, &newNodeNum, &newIndex, &leftNode, &updateParent, &insertParent,
		    &temp);
		M_ExitOnError(err);
	...
	if (insertParent || updateParent) {
		...
		err = InsertLevel(btreePtr, treePathTable, primaryKey, secondaryKey, &parentNode, index, level, insertNode);
		M_ExitOnError(err);
	}
...
static OSErr
InsertNode(BTreeControlBlockPtr btreePtr, InsertKey *key,

    BlockDescriptor *rightNode, UInt32 node, UInt16 index,

    UInt32 *newNode, UInt16 *newIndex,
    ...
    BlockDescriptor *leftNode, Boolean *updateParent, Boolean *insertParent, Boolean *rootSplit)
{
	...
	*rootSplit = false;
	...
	if (!recordFit && leftNodeNum > 0) {
		PanicIf(leftNode->buffer != nil, " InsertNode: leftNode already aquired!");
		...
		{
			err = RotateLeft(btreePtr, leftNode->buffer, rightNode->buffer, index, key->keyPtr, key->recPtr, key->recSize, newIndex, newNode,
			    &recordFit, &recsRotated);
			M_ExitOnError(err);
	...
	if (!recordFit) {
		...
		err = SplitLeft(btreePtr, leftNode, rightNode, node, index, key->keyPtr, key->recPtr, key->recSize, newIndex, newNode, &recsRotated);
		M_ExitOnError(err);
...
OSStatus
DeleteTree(BTreeControlBlockPtr btreePtr, TreePathTable treePathTable, BlockDescriptor *targetNode, UInt16 index, UInt16 level)
{
	OSStatus err;
	BlockDescriptor parentNode;
	...
	if (updateRequired || deleteRequired) {
		...
		if (updateRequired) {
			...
			err = InsertTree(btreePtr, treePathTable, keyPtr, recPtr, recSize, &parentNode, index, level, kReplaceRecord, &insertNode);
			M_ExitOnError(err);
		} else // deleteRequired
		{
...
static OSStatus
CollapseTree(BTreeControlBlockPtr btreePtr, BlockDescriptor *blockPtr)
{
	OSStatus err;
	UInt32 originalRoot;
	UInt32 nodeNum;
...
static OSStatus
RotateLeft(BTreeControlBlockPtr btreePtr, NodeDescPtr leftNode, NodeDescPtr rightNode, UInt16 rightInsertIndex, KeyPtr keyPtr, UInt8 *recPtr, UInt16 recSize,
    UInt16 *insertIndex, UInt32 *insertNodeNum, Boolean *recordFit, UInt16 *recsRotated)
{
	OSStatus err;
	SInt32 insertSize;
	SInt32 nodeSize;
	...
	*recordFit = true;
	...
	while (index < moveIndex) {
		if (index == rightInsertIndex) // insert new record in left node
		{
			...
			didItFit = InsertKeyRecord(btreePtr, leftNode, leftInsertIndex, keyPtr, keyLength, recPtr, recSize);
			if (!didItFit) {
	...
	if (moveIndex <= rightInsertIndex) // then insert new record in right node
	{
		...
		didItFit = InsertKeyRecord(btreePtr, rightNode, rightInsertIndex, keyPtr, keyLength, recPtr, recSize);
		if (!didItFit) {
...
static OSStatus
SplitLeft(BTreeControlBlockPtr btreePtr, BlockDescriptor *leftNode, BlockDescriptor *rightNode, UInt32 rightNodeNum, UInt16 index, KeyPtr keyPtr, UInt8 *recPtr,
    UInt16 recSize, UInt16 *insertIndex, UInt32 *insertNodeNum, UInt16 *recsRotated)
{
	OSStatus err;
	...
	err = RotateLeft(btreePtr, left, right, index, keyPtr, recPtr, recSize, insertIndex, insertNodeNum, &recordFit, recsRotated);
...
static Boolean
RotateRecordLeft(BTreeControlBlockPtr btreePtr, NodeDescPtr leftNode, NodeDescPtr rightNode)
{
	UInt16 size;
	UInt8 *recPtr;
	Boolean recordFit;
...
static OSStatus
AddNewRootNode(BTreeControlBlockPtr btreePtr, NodeDescPtr leftNode, NodeDescPtr rightNode)
{
	OSStatus err;
	...
	didItFit = InsertKeyRecord(btreePtr, rootNode.buffer, 0, keyPtr, keyLength, (UInt8 *)&rightNode->bLink, 4);
	...
	didItFit = InsertKeyRecord(btreePtr, rootNode.buffer, 1, keyPtr, keyLength, (UInt8 *)&leftNode->fLink, 4);
...
static UInt16
GetKeyLength(const BTreeControlBlock *btreePtr, const BTreeKey *key, Boolean forLeafNode)
{
	UInt16 length;
