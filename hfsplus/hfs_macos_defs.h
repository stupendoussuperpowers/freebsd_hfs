...
#ifdef __FreeBSD__
/*
 * Glue and hacks to make the code build and work in FreeBSD
 */
#define __private_extern__
#define current_thread() curthread
#define current_proc() curthread
#define meta_bread bread
#define KERNEL_DEBUG(a,b,c,d,e,f) /* no-op */
#define MAXPHYSIO MAXPHYS
#define MALLOC_ZONE		malloc
#define MALLOC	malloc
#define FREE_ZONE(a, b, c)	free((a), (c))
#define VOP_ABORTOP(vp, cnp) /* no-op */
#define	VOP_DEVBLOCKSIZE(devvp, sizep) (*(sizep) = DEV_BSIZE) /* YYY */
#define VOP_BWRITE bwrite
#define VOP_TRUNCATE hfs_truncate
#define VOP_UPDATE hfs_update
#define MNT_UNKNOWNPERMISSIONS MNT_ACLS	/* YYY temporary!!! */
#define SET(t, f)	((t) |= (f))
#define CLR(t, f)	((t) &= ~(f))
#define ISSET(t, f)	((t) & (f))
typedef struct thread proc_t;

#if __FreeBSD_version >= 500105
  #define _GETBLK(a, b, c, d, e) getblk((a), (b), (c), (d), (e), 0)
#else
  #define _GETBLK(a, b, c, d, e) getblk((a), (b), (c), (d), (e))
#endif
#if __FreeBSD_version >= 500102
  #define _BUF_LOCK(a, b) BUF_LOCK((a), (b), NULL)
#else
  #define _BUF_LOCK(a, b) BUF_LOCK((a), (b))
#endif
