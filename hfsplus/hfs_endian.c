 ...
 */
void
hfs_swap_HFSPlusForkData(HFSPlusForkData *src)
{
	int i;
 ...
 */
int
hfs_swap_BTNode(BlockDescriptor *src, int isHFSPlus, HFSCatalogNodeID fileID, int unswap)
{
	...
	if ((srcDesc->kind == kBTIndexNode) || (srcDesc->kind == kBTLeafNode)) {
		if (isHFSPlus) {
...
int
hfs_swap_HFSPlusBTInternalNode(BlockDescriptor *src, HFSCatalogNodeID fileID, int unswap)
{
	BTNodeDescriptor *srcDesc = src->buffer;
	...
	if (fileID == kHFSExtentsFileID) {
		...
		for (i = 0; i < srcDesc->numRecords; i++) {
			...
			} else if ((srcPtr[0] == kHFSPlusFolderThreadRecord) || (srcPtr[0] == kHFSPlusFileThreadRecord)) {
...
int
hfs_swap_HFSBTInternalNode(BlockDescriptor *src, HFSCatalogNodeID fileID, int unswap)
{
	BTNodeDescriptor *srcDesc = src->buffer;
	UInt16 *srcOffs = (UInt16 *)((char *)src->buffer + (src->blockSize - (srcDesc->numRecords * sizeof(UInt16))));
	...
	if (fileID == kHFSExtentsFileID) {
		...
		for (i = 0; i < srcDesc->numRecords; i++) {
			...
			} else if ((srcPtr[0] == kHFSFolderThreadRecord) || (srcPtr[0] == kHFSFileThreadRecord)) {
